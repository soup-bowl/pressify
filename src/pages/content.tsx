...
const Content = ({ posts, pages }: Props) => {
	...
	const saveResponse = (p: IPost) => {
		...
		if (p.type === "page") {
			wp.fetchPosts({ type: EPostType.Page, parent: p.id }).then((c: IPostCollection) => setChildren(c.posts))

			if (p.parent !== undefined && p.parent !== 0) {
				wp.fetchPost(p.parent, EPostType.Page).then((c: IPost) => setParent(c))
			}
		}
	...
	return (
		<Box>
			<Typography variant="h1">{degubbins(post.title.rendered)}</Typography>
			<StyledStack my={2} spacing={2} color="darkgrey">
				...
				{post.link !== undefined && <OriginalContentLink url={post.link} />}
			...
			<Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
				...
				{post._embedded !== undefined &&
					...
					post._embedded["wp:term"][1] !== undefined && (
						<Grid item xs={12} sm={6}>
							<TagGrid title="Tags" tags={post._embedded["wp:term"][1]} urlFormat={`/${inputURL}/posts/tag`} />
						</Grid>
					)}
			</Grid>
		</Box>
