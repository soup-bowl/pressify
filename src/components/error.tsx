...
export const GeneralAPIError = ({ endpoint = "Unspecified", message = "", noheader = false }: Props) => {
	return (
		<Box>
			...
			<Typography my={2} sx={{ fontFamily: "monospace" }}>
				{message}
			</Typography>
		</Box>
	)
...
export const PrincipalAPIError = ({ message = "" }: PrinciProps) => {
	return (
		<Box>
			...
			<Typography my={2}>
				We've made a request to the website's JSON API (if it exists), and have not had the expected response returned.
			</Typography>
			<Typography my={2}>This can be due to the following reasons:</Typography>
			<ul>
